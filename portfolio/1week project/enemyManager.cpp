...
void enemyManager::update()
{
	...
	if (m_vBoss.size() > 0)
	{
		...
		if (count >= 1700)
		{
			...
			if (count >= 8250 && count < 10000)
			{
				bossBulletFire(19);
				bossBulletFire(0);
				bossBulletFire(1);
			}
			if (count >= 10000 && m_vBoss[0]->getHp() > 0)
			{
				isGameOver = true;
				m_vBoss[0]->move(IDLE);
				m_vBoss[0]->move(UP);
...
void enemyManager::statusRender()
{
	...
	sprintf_s(str, "보스HP :  %d", m_vBoss[0]->getHp());
	TextOut(getMemDC(), 740, 510, str, strlen(str));

	if (KEYMANAGER->isToggleKey(VK_F3))
	{
		sprintf_s(str, "레드미니언 사이즈 :  %d", m_vRedMinion.size());
		TextOut(getMemDC(), 740, 430, str, strlen(str));

		sprintf_s(str, "옐로미니언 사이즈 :  %d", m_vYellowMinion.size());
		TextOut(getMemDC(), 740, 450, str, strlen(str));

		sprintf_s(str, "그린미니언 사이즈 :  %d", m_vGreenMinion.size());
		TextOut(getMemDC(), 740, 470, str, strlen(str));

		sprintf_s(str, "블루미니언 사이즈 :  %d", m_vBlueMinion.size());
		TextOut(getMemDC(), 740, 490, str, strlen(str));
	}
}
...
void enemyManager::setRedMinion(int pattern)
{
	switch (pattern)
	{
		...
		case 1:
			...
			{
				m_redMinion = new redMinion;
				m_redMinion->init("redMinion", PointMake(175 + (i * 250), -150));
				m_vRedMinion.push_back(m_redMinion);
			}			
			for (int i = 2; i < 5; i++)
			{
				m_redMinion = new redMinion;
				m_redMinion->init("redMinion", PointMake(50 + ((i - 2) * 250), -50));
				m_vRedMinion.push_back(m_redMinion);
			}
		default:
			break;
...
void enemyManager::setBlueMinion(int pattern)
{
	...
	case 1:
	{
		...
		{
			m_blueMinion = new blueMinion;
			m_blueMinion->init("blueMinion", PointMake(475 + (i * 250), -150));
			m_vBlueMinion.push_back(m_blueMinion);
		}
		for (int i = 2; i < 5; i++)
		{
			m_blueMinion = new blueMinion;
			m_blueMinion->init("blueMinion", PointMake(350 + ((i - 2) * 250), -50));
			m_vBlueMinion.push_back(m_blueMinion);
		}
	}
	default:
...
void enemyManager::setGreenMinion(int pattern)
{
	...
	case 2:
	{
		...
		{
			m_greenMinion = new greenMinion;
			m_greenMinion->init("greenMinion", PointMake(-500 + i * 100, 400));
			m_vGreenMinion.push_back(m_greenMinion);
		}
		for (int i = 5; i < 10; i++)
		{
			m_greenMinion = new greenMinion;
			m_greenMinion->init("greenMinion", PointMake(-500 + (i - 5) * 100, 500));
			m_vGreenMinion.push_back(m_greenMinion);
		}
	}
	default:
...
void enemyManager::greenMinionBulletFire(int pattern)
{
	...
	case 4:
	{
		for (m_viGreenMinion = m_vGreenMinion.begin(); m_viGreenMinion != m_vGreenMinion.end(); ++m_viGreenMinion)
		{
			BULLETMANAGER->minionSpinBulletFire(2, "greenSmallBullet", (*m_viGreenMinion)->getGreenMinionX(), (*m_viGreenMinion)->getGreenMinionY(), 5);
		}
	}
	break;
	default:
...
void enemyManager::setYellowMinion(int pattern)
{
	...
	case 1:
	{
		...
		{
			m_yellowMinion = new yellowMinion;
			m_yellowMinion->init("yellowMinion", PointMake(950 + (i * 100), 400));
			m_vYellowMinion.push_back(m_yellowMinion);
		}
		for (int i = 5; i < 10; i++)
		{
			m_yellowMinion = new yellowMinion;
			m_yellowMinion->init("yellowMinion", PointMake(950 + ((i-5) * 100), 500));
			m_vYellowMinion.push_back(m_yellowMinion);
		}
	}
	break;
...
void enemyManager::yellowMinionBulletFire(int pattern)
{
	...
	case 1:
	{
		for (m_viYellowMinion = m_vYellowMinion.begin(); m_viYellowMinion != m_vYellowMinion.end(); ++m_viYellowMinion)
		{
			BULLETMANAGER->minionSpinBulletFire(3, "yellowSmallBullet", (*m_viYellowMinion)->getYellowMinionX(), (*m_viYellowMinion)->getYellowMinionY(), 5);
		}
	}
	default:
		break;
