...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	*/
	...
	case WM_PAINT:		// 윈도우의 작업영역이 손실되었을때 실행된다. 그리는곳
		...
		rc2 = RectMakeCenter(m_ptMouse.x, m_ptMouse.y, 50, 50);
	...
	case WM_MOUSEMOVE:
		...
		{
			if (rc.left >= m_ptMouse.x) // 왼쪽에서 미는 것
			{
				rc.left -= MOVERECT;
				rc.right -= MOVERECT;
			}
			if (rc.right <= m_ptMouse.x) // 오른쪽에서 미는 것
			{
				rc.left += MOVERECT;
				rc.right += MOVERECT;
			}
			if (rc.top >= m_ptMouse.y) // 위에서 미는 것
			{
				rc.top -= MOVERECT;
				rc.bottom -= MOVERECT;
			}
			if (rc.bottom <= m_ptMouse.y) // 아래에서 미는 것
			{
				rc.top += MOVERECT;
				rc.bottom += MOVERECT;
			}
