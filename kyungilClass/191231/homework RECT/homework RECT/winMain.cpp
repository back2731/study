...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	*/
	...
	case WM_PAINT:		// 윈도우의 작업영역이 손실되었을때 실행된다. 그리는곳
		...
		wsprintf(str, "%d : %d : %d", currentHour, currentMinute, currentSecond);
	...
	case WM_MOUSEMOVE:
		...
		{
			Rectangle(hdc, rc.left, rc.top, rc.right, rc.bottom);
			rc = RectMakeCenter(m_ptMouse.x, m_ptMouse.y, 100, 100);		
		}

		if (drag)
		{
			if ((rc2.left + rc2.right) / 2 <= m_ptMouse.x && (rc.top + rc.bottom) / 2 <= m_ptMouse.y)
			{
				rc2.right = m_ptMouse.x;
				rc2.bottom = m_ptMouse.y;
			}
			if ((rc2.left + rc2.right) / 2 <= m_ptMouse.x && (rc.top + rc.bottom) / 2 >= m_ptMouse.y)
			{
				rc2.right = m_ptMouse.x;
