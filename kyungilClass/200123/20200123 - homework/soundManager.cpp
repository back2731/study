...
void soundManager::sound(float volume)
{
	int count = 0;
	...
	for (iter; iter != m_totalSounds.end(); ++iter, count++)
	{
		m_channel[count]->setVolume(currentVolume + volume);
...
void soundManager::mute()
{
	...
	for (iter; iter != m_totalSounds.end(); ++iter, count++)
	{
		m_channel[count]->setVolume(0.0f);
	}
}
...
void soundManager::setVolum()
{
	...
	for (iter; iter != m_totalSounds.end(); ++iter, count++)
	{
		m_channel[count]->setVolume(currentVolume);
	}
}
