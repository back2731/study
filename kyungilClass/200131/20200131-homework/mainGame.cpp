...
void mainGame::maptoolSetup()
{
	...
	_saveMap1 = CreateWindow("button", "saveMap1", WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON, 1000, 350, 100, 30, m_hWnd, HMENU(5), m_hInstance, NULL);
	_saveMap2 = CreateWindow("button", "saveMap2", WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON, 1000, 400, 100, 30, m_hWnd, HMENU(6), m_hInstance, NULL);
	_saveMap3 = CreateWindow("button", "saveMap3", WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON, 1000, 450, 100, 30, m_hWnd, HMENU(7), m_hInstance, NULL);
...
void mainGame::save()
{
	if (_ctrSelect == CTRL_MAP1)
	{
		name = "map1.map";
	}
	if (_ctrSelect == CTRL_MAP2)
	{
		name = "map2.map";
	}
	if (_ctrSelect == CTRL_MAP3)
	{
		name = "map3.map";
	}
...
void mainGame::load()
{
	...
	ReadFile(file, _tiles, sizeof(tagTile) *TILEX* TILEY, &lead, NULL);
	CloseHandle(file);
}
